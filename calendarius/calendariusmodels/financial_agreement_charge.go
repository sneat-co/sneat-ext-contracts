...
func (f FinancialAgreementChargeFact) Validate() error {
	...
	if f.Status == FinancialChargeStatusAvailable {
		if f.AmountMinor == nil || *f.AmountMinor < 1 || *f.AmountMinor > MaxJavaScriptSafeInteger || (f.Direction != FinancialChargeDirectionExpense && f.Direction != FinancialChargeDirectionIncome && f.Direction != FinancialChargeDirectionTransfer) {
